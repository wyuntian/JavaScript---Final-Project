...
var resetMarkers = function() {
  _.each(marker._layers, function(marker) {
    marker.setStyle({color: '#ff7800'});
  });
};
// Set up click event for when the user clicks the sidebar
var sidebarClick = function(id) {
  // Reset all markers to original color
  resetMarkers();
  // Change only the marker with the id matching the sidebar
  rec._layers.setStyle({color: '#ffffcc'});
};
console.log(rec);
// Add items on sidebar
// _.each(markered._layers, function(marker) {
//   $('.sidebar').append("<li class='item'  onclick='sidebarClick("+rec._leaflet_id+")'>"+rec.facilname_label+"</li>");
// });
}
